...
def read_serial_numbers(params_yaml_fp: str) -> Dict[str, List[str]]:
    ...
    with open(params_yaml_fp, "r") as yaml_file:
        ...
        for camera_model, serial_list in serials_dict.items():
            model_serials[camera_model] = [serial for serial in serial_list if serial]
    return model_serials


def add_visionary_pub_to_launch_description(
    launch_description: LaunchDescription, model: str, serial: str
):
    """
...
def add_pointcloud_pub_to_launch_description(
...
def create_robot_state_pub_description(model: str, serial: str) -> ComposableNode:
    ...
    xacro_path = os.path.join(
        get_package_share_directory("sick_visionary_gev_ros2"), "meshes", xacro_file_name
    )
    robot_description_config = xacro.process_file(
        xacro_path, mappings={"link_name": "camera_frame_" + serial}
    ).toxml()
...
def generate_launch_description():
    ...
    config = os.path.join(
        get_package_share_directory("sick_visionary_gev_ros2"), "config", "params.yaml"
    )
    ...
    for model, serials in models_serials_dict.items():
        for serial in serials:
            ...
            add_pointcloud_pub_to_launch_description(
                launch_description, publish_pointcloud, model, serial
            )
